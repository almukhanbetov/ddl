...
<form method="POST"
    ...
    {{-- Название --}}
    <input name="name"
           class="w-full bg-slate-800 border p-2 rounded"
...
<form method="POST"
      action="{{ route('admin.products.gallery.upload', $product) }}"
      enctype="multipart/form-data"
      class="mb-6">
    ...
    @foreach($product->gallery ?? [] as $i => $img)
        ...
        <div class="relative">
            ...
            <img src="{{ asset('storage/'.$img) }}"
                 class="rounded border border-slate-700 h-32 w-full object-cover">

            {{-- кнопка удаления --}}
            <form method="POST"
                  action="{{ route('admin.products.gallery.delete', [$product->id, $i]) }}"
